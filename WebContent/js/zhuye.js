  ...
  function getData(bt){ 
	...
	$.ajax({
		//请求路径
		url:"http://localhost:8080/CRM/User2",
		//请求数据
	    ...
	    success:function(data){
	    	...
	    	if(data.flag == "true"){
	    	  ...
	    	  for(var i=0;i<datas.length;i++){
	    		...
	    		'<button class="add" onclick="xiugai('+datas[i].id+')">修改</button>'+
	    		'</td>'+
	    		'<tr>';
	    	  }
	    	$("#tab").html(table);
	...
	function getData2(){
		...
		$.ajax({
			//请求路径
			url:"http://localhost:8080/CRM/chaxun",
			//请求数据
			data:{page1: $("#id1").val()},
			//传输数据l
			 dataType:"json",
    ...
    function getData4(){
    	...
    	$.ajax({
			//请求路径
			url:"http://localhost:8080/CRM/chaxun2",
			//请求数据
			data:{page1: $("#id1").val()},
			//传输数据l
			 dataType:"json",
